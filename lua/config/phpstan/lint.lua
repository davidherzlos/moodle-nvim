...
local phpstan = {
  ...
  cmd = function ()
    ...
    return (vim.uv or vim.loop).fs_stat(local_bin) and local_bin or bin
  end,
  args = {
    'analyze',
    '--memory-limit=512M',
...
function M.try_lint()
  local use_linter = function(_) return true end
  local linter = phpstan
  assert(linter, 'Linter phpstan not available')
...
function LintProc:start_read()
  ...
  if type(parser) == 'function' then
    parser = require('config.phpstan.parser').accumulate_chunks(parser)
  end
  assert(
  ...
  elseif stream == 'both' then
    local parser1, parser2 = require('config.phpstan.parser').split(parser)
